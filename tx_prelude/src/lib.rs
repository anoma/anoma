...
impl TxEnv for Ctx {
    ...
    fn has_key(&self, key: &anoma::types::storage::Key) -> Result<bool, Error> {
        let key = key.to_string();
        let found =
    ...
    fn get_block_height(
        &self,
    ) -> Result<anoma::types::storage::BlockHeight, Error> {
        Ok(BlockHeight(unsafe { anoma_tx_get_block_height() }))
    }

    fn get_block_hash(
        &self,
    ) -> Result<anoma::types::storage::BlockHash, Error> {
    ...
    fn get_block_epoch(&self) -> Result<anoma::types::storage::Epoch, Error> {

defmodule AnomaTest.Node.Executor do
  ...
  setup_all do
    ...
    {:ok, ordering} = Router.start_engine(router, Anoma.Node.Storage.Ordering, table: storage)

    snapshot_path = [:my_special_nock_snaphsot | 0]
    env = %Nock{snapshot_path: snapshot_path, ordering: ordering}

    {:ok, executor} = Router.start_engine(router, Anoma.Node.Executor, {env, Router.new_topic(router)})

defmodule AnomaTest.Node.Mempool do
  ...
  setup_all do
    ...
    {:ok, nodes} = Anoma.Node.start_link(
      name: name,
  ...
  test "successful process", %{node: node} do
    ...
    :ok = Router.call(node.router, {:subscribe_topic, node.executor_topic.id, :local})
    ...
    :ok = Router.call(node.router, {:unsubscribe_topic, node.executor_topic.id, :local})
  end
  ...
  test "Transaction print", %{node: node} do
    ...
    zero_tx = Mempool.tx(node.mempool, zero_counter(key))
  ...
  test "Transactions broadcast", %{node: node} do
    :ok = Router.call(node.router, {:subscribe_topic, node.mempool_topic.id, :local})

    Mempool.hard_reset(node.mempool)

    zero_tx = Mempool.tx(node.mempool, zero_counter(777))

    assert_receive {:"$gen_cast", {_, {:submitted, ^zero_tx}}}
    Mempool.soft_reset(node.mempool)
    :ok = Router.call(node.router, {:unsubscribe_topic, node.mempool_topic.id, :local})
  end
end

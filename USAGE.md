...
The Anoma client serves the purpose of taking in request from the third party client and relaying them 
...
```text 
~/Documents/Work/anoma  (testnet-01) % ./apps/anoma_client/anoma_client --listen-port 50051 --node-host localhost --node-port 58615
...
The prove API is the most important one for the Juvix team. 

The corresponding proto definition is as follows.
...
A request can either be a jammed program, or a textual representation of a noun (e.g., `"[1 2]"`).
The inputs are a list of jammed nouns, or textual nouns. They cannot be both.

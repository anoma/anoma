defmodule Nock.Jets do
  ...
  def verify_detatched(core) do
    ...
    case maybe_sample do
      {:ok, [a, b | c]} when is_binary(a) and is_binary(b) and is_binary(c) ->
        try do
          if Sign.verify_detached(a, b, c) do
            {:ok, 0}
          else
            {:ok, 1}
          end
  ...
  def verify(core) do
    ...
    case maybe_sample do
      {:ok, [a | b]} when is_binary(a) and is_binary(b) ->
        try do
          case Sign.verify(a, b) do
            {:ok, val} -> {:ok, val}
            {:error, _} -> :error
          end
        rescue
  ...
  @spec on_binary((binary(), binary() -> binary()), Nock.t()) ::
          :error | {:ok, binary()}

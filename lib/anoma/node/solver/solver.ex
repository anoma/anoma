defmodule Anoma.Node.Solver.Solver do
  ...
  @spec add_intent(GenServer.server(), Anoma.Transaction) :: list(Anoma.Transaction)
  def add_intent(server, intent) do
    GenServer.call(server, {:add_intent, intent})
  end
  @spec del_intents(GenServer.server(), Anoma.Transaction) :: list(Anoma.Transaction)
  ...
  defp handle_solve({unsolved, solved}) do
    ...
    {:reply, Enum.map(new_solved, &hd/1), {new_unsolved, new_solved ++ solved}}
  end
  ...
  def handle_call({:del_intents, deleted}, _from, {unsolved, solved}) do
    ...
    {nolonger_solved, solved} = Enum.split_with(solved, fn x -> Enum.any?(tl(x), fn x -> x in deleted end) end)
    nolonger_solved = nolonger_solved
                      |> Enum.map(&tl/1)
                      |> Enum.concat
  ...
  defp solve(unbalanced, balanced, n) do
    if n < 0 do
      ...
      {_, selected, unselected} = Enum.reduce(unbalanced, {1, [], []},
        fn el, {i, selected, unselected} ->
          if (i &&& n) == 0 do
      ...
      composed_selected = Enum.reduce(selected, fn x, y -> x && y && Anoma.Transaction.compose(x, y) end)
      if composed_selected && Anoma.Transaction.verify(composed_selected) do
        # got a matc

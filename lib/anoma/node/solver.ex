defmodule Anoma.Node.Solver do
  ...
  defp handle_solve(s) do
    ...
    if new_solved != [] do
      Router.cast(s.solutions_topic, {:solutions, Enum.map(new_solved, &hd/1)})
    end
    solved_set = Enum.reduce(new_solved, s.solved_set,
      fn [_ | intents], set ->
        Enum.reduce(intents, set, fn intent, set -> MapSet.put(set, intent) end)
      end)
    {:noreply, %{s | unsolved: new_unsolved, solved: new_solved ++ s.solved, solved_set: solved_set}}
  end
  ...
  def handle_cast({:remove_intent, deleted}, _, s) do
    ...
    {nolonger_solved, still_solved} = Enum.split_with(s.solved, fn x -> deleted in tl(x) end)
    ...
    handle_solve(%{s | unsolved: unsolved ++ nolonger_solved, solved: still_solved, solved_set: solved_set})
  end

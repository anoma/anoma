defmodule Anoma.Node.Storage do
  ...
  def handle_call({:new_order, ordered_transactions}, _from, state) do
    ...
    {:reply, :ok,
     %{state | next_order: new_next_order, hash_to_order: new_map}}
  end
      ...
      [_ | _] ->
        ...
        case result do
          ...
          [] ->
            receive do
              {:mnesia_table_event,
               {:write, {Anoma.Node.Storage.Qualified, ^key, value}, _}} ->
                {:ok, value}
            end
        end
  ...
  def get(key) do
    ...
    case maybe_order do
      [{_, ^key, order}] ->
        ...
        value_tx = fn ->
          :mnesia.read(Anoma.Node.Storage.Qualified, [order, key | 0])
        end
  ...
  def put(key, value) do
    ...
    write_tx = fn ->
      ...
      :mnesia.write(
        {Anoma.Node.Storage.Qualified, [new_order, key | 0], value}
      )
    end
  ...
  def create_tables() do
    ...
    :mnesia.create_table(Anoma.Node.Storage.Qualified,

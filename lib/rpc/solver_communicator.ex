defmodule RPC.SolverCommunicator do
  ...
  def init({solver_com, validator_host, validator_port}) do
    ...
    {:ok, validator_conn} = GRPC.Stub.connect(validator_host <> ":" <> to_string(validator_port))
  ...
  def handle_cast({:add_intent, intent}, {solver_com, validator_conn}) do
    Solver.Communicator.add_intent(solver_com, RPC.Convert.deserialise_transaction(intent))
    {:noreply, {solver_com, validator_conn}}
  end
  def handle_cast({:del_intent, intent}, {solver_com, validator_conn}) do
    Solver.Communicator.del_intents(solver_com, [RPC.Convert.deserialise_transaction(intent)])
    {:noreply, {solver_com, validator_conn}}
  end

  def handle_cast({:solutions, solutions}, {solver_com, validator_conn}) do
    Enum.each(solutions, fn x -> AnomaInterface.Validator.Stub.propose_transaction(validator_conn, RPC.Convert.serialise_transaction(x)) end)
    {:noreply, {solver_com, validator_conn}}
  end
end

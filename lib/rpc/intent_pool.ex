defmodule RPC.IntentPool do
  ...
  def listen_intents(_empty, stream) do
    ...
    stream = GRPC.Server.send_reply(stream, %AnomaInterface.Transaction{roots: [%AnomaInterface.MerkleAnchor{hash: "aaa"}]})
    #GRPC.Server.send_reply(stream, %AnomaInterface.Transaction{})
    stream = GRPC.Server.send_reply(stream, %AnomaInterface.Transaction{roots: [%AnomaInterface.MerkleAnchor{hash: "bbb"}]})

...
pub mod main {
    ...
    fn apply_tx(tx_data: Vec<u8>) {
        ...
        let write_op = match transaction::util::WriteOp::try_from_slice(&data[..]) {
            Ok(write_op) => {
        ...
        match val {
            Some(val) => {
                log(&format!("storage key already has a value present- {} bytes", val.len()));
            }
            None => {
        ...
        log(&format!(
            ...
            write_op.value.len(), &write_op.key
        ));

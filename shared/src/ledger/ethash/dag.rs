...
impl LightDAG {
    ...
    ///     blocks ahead. It does this asychronously in the background
    ...
    pub fn update(&mut self, number: u64) {
        // update the epoch
        self.epoch =
            std::cmp::max((number / EPOCH_LENGTH as u64) as usize, self.epoch);
    ...
    /// If the block number is `self.update_ahead` blocks from the next epoch,
    /// begin pre-computing the next cache in the background
    ...
    /// If the current block number is at least `self.keep_previous` blocks past
    /// the last epoch change, drop the previous cache. It is no longer
    /// needed.
    ...
    pub fn verify_header(
        ...
        let (mix, final_hash) = if epoch + 1 == self.epoch {
            match self.previous_cache.as_ref() {
                ...
                None => self.compute_slow(hash.into(), nonce.into(), number),
            }

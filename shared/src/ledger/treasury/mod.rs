...
use crate::types::address::xan as nam;
        ...
        let result = keys_changed.iter().all(|key| {
            ...
            match key_type {
                ...
                KeyType::BALANCE(addr) => {
                    ...
                    let is_max_funds_transfer_key = treasury_storage::get_max_transferable_fund_key();
                    let balance_key = token::balance_key(&nam(), &ADDRESS);
                    let max_transfer_amount = self.ctx.read_pre(&is_max_funds_transfer_key);
                    ...
                    match (max_transfer_amount, pre_balance, post_balance) {
                        (Ok(max_transfer_amount), Ok(pre_balance), Ok(post_balance)) => {
                            match (max_transfer_amount, pre_balance, post_balance) {
                                (Some(max_transfer_amount), Some(pre_balance), Some(post_balance)) => {
                                    let max_transfer_amount = token::Amount::try_from_slice(&max_transfer_amount[..]).ok();
                                    let pre_balance = token::Amount::try_from_slice(&pre_balance[..]).ok();
                                    let post_balance = token::Amount::try_from_slice(&post_balance[..]).ok();
                                    match (max_transfer_amount, pre_balance, post_balance) {
                                        (Some(max_transfer_amount), Some(pre_balance), Some(post_balance)) => {
                                            post_balance > pre_balance || (pre_balance - post_balance <= max_transfer_amount)
                                        },
                                        _ => return false
                                    }
                                },

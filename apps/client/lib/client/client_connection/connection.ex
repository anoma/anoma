defmodule Anoma.Client.Connection.TCP do
  ...
  typedstruct do
    ...
    field(:socket, port())
    field(:node_id, Id.t())
    field(:remote_node_id, NodeInfo.t())
  ...
  @impl true
  def init(args) do
    Logger.debug("#{inspect(self())} :: startig client connection")
    args = Keyword.validate!(args, [:socket, :node_id])
  ...
  # @doc """
  # When the remote node announces itself, I set the remote id in this connection.
  # """
  def handle_cast({:set_remote_node_id, remote_node_id}, state) do
    {:noreply, %{state | remote_node_id: remote_node_id}}
  end
  ...
  # """
  @spec handle_tcp_in(any(), Id.t()) :: :ok
  ...
  # """
  @spec handle_tcp_out(Enveloppe.t(), port()) :: :ok
  ...
  # """
  @spec decode(binary()) :: {:ok, Enveloppe.t()} | {:error, :invalid_message}
  defp decode(bytes) do
    try do
      {:ok, Enveloppe.decode(bytes)}
    rescue
  ...
  # @doc """
  # I encode a protobuf enveloppe into bytes.
  # """
  @spec encode(Enveloppe.t()) :: binary()
  defp encode(%Enveloppe{} = message) do
  ...
  # """
  defp handle_message_in(%Enveloppe{} = message, local_node_id) do
  ...
  defp handle_message_in(%Announcement{} = announcement, _ref, _local_node_id) do
    ...
    |> Enum.each(
      &GenServer.cast(self(), {:engine_discovered, &1, remote_node_id})
    )
  ...
  defp handle_message_in(message, ref, local_node_id) do
    ...
    case Messages.proto_to_call(message, ref, local_node_id) do
      {:reply, enveloppe} ->
        GenServer.cast(self(), {:tcp_out, enveloppe})
        :ok

defmodule Anoma.LocalDomain.Examples.ELocalDomain do
  ...
  """
  @spec create_example_client(ENode.t() | nil) :: ELocalDomain.t()
  def create_example_client(enode \\ create_single_example_node()) do
    ...
    DynamicSupervisor.which_children(Anoma.LocalDomain.ConnectionSupervisor)
    |> Enum.each(fn {_, pid, _, _} ->
      DynamicSupervisor.terminate_child(
        Anoma.LocalDomain.ConnectionSupervisor,
        pid
      )
    end)

    {:ok, client} = LocalDomain.connect("localhost", grpc_port, enode.node_id)

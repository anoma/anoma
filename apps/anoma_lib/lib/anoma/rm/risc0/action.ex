defmodule Anoma.RM.Risc0.Action do
  @moduledoc """
  I represent an action in the risc0 resource machine.
  """
  ...
  def verify(action) do
    with true <- verify_proofs(action.logic_proofs),
      ...
      logic_instances = Enum.map(action.logic_proofs, &LogicInstance.from_proof/1)

      # Decode compliance_instance from compliance_proofs
      compliance_instance = Enum.map(action.compliance_proofs, &ComplianceInstance.from_proof/1)

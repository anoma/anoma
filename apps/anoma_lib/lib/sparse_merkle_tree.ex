defmodule SparseMerkleTree do
  ...
  """
  @spec root_hash(Self.t()) :: {:ok, {hash(), integer()}} | {:error, StaleTreeError.t()}
  def root_hash(%Self{} = self) do
    case cached_subtree_hash(self.root) do
      {:ok, hash} -> {:ok, {hash, self.depth}}
      :none -> {:error, %StaleTreeError{}}
    end
  end

  @spec root_hash!(Self.t()) :: {hash(), integer()}
  ...
  def valid_proof?(
    ...
    cond do
      max_tree_depth > depth ->
        # Proof length must not exceed tree depth
        false
      presence == :present && max_tree_depth < depth ->
        # Presence proof length must equal tree depth
        false
      true ->
        # Otherwise the proof at least has the correct structure

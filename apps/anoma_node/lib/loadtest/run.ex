defmodule Anoma.Node.Loadtest.Run do
  ...
  def run() do
    ...
    client = Peer.create_client(node)

    Process.sleep(1000)

    # totla transactions to submit
    iterations = 1000

    # measure the average execution time of submitting a transaction
    {average_time_μs, _} = :rpc.block_call(client.name, Anoma.Node.Loadtest.Run, :measure_average_transaction_time, [iterations])

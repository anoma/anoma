defmodule Anoma.Node.Examples.ETransaction do
  ...
  def zero_counter_submit(node_id \\ Node.example_random_id()) do
    ...
    assert_receive(
      {:mnesia_table_event,
       {:write, {^blocks_table, ["anoma", "block", 0], _}, _}},
      5000
    )
    ...
    {:atomic, block} =
      :mnesia.transaction(fn ->
        :mnesia.read({Storage.blocks_table(node_id), ["anoma", "block", 0]})
      end)
  ...
  def inc_counter_submit_with_zero(node_id \\ Node.example_random_id()) do
    ...
    {:atomic, block} =
      :mnesia.transaction(fn ->
        :mnesia.read({blocks_table, ["anoma", "block", 0]})
      end)
    ...
    [
      {^blocks_table, ["anoma", "block", 0], [^tx1, ^tx2]}
    ] = block
  ...
  def bluf_transaction_errors(node_id \\ Node.example_random_id()) do
    ...
    [
      {^blocks_table, ["anoma", "block", 0], [^tx1]}
    ] = block

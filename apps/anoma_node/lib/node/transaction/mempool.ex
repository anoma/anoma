defmodule Anoma.Node.Transaction.Mempool do
  ...
  def init(args) do
    ...
    {:ok, %__MODULE__{round: args[:round], node_id: node_id},
     {:continue, {:rebuild_state, args[:consensus], args[:transactions]}}}
  end

  @impl true
  def handle_continue({:rebuild_state, consensus, transactions}, state) do
    # Process.sleep(500)
    for {id, tx_w_backend} <- transactions do
      tx(state.node_id, tx_w_backend, id)
    end

    for list <- consensus do
      execute(state.node_id, list)
    end

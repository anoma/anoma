defmodule Anoma.Node.Transaction.Storage do
  ...
  def init(args) do
    ...
    values_table =
      String.to_atom("#{keylist[:values_table]}_#{:erlang.phash2(node_id)}")

    updates_table =
      String.to_atom("#{keylist[:updates_table]}_#{:erlang.phash2(node_id)}")

    blocks_table =
      String.to_atom("#{keylist[:blocks_table]}_#{:erlang.phash2(node_id)}")

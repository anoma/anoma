defmodule Anoma.Node.Transport.TCP.Listener do
  ...
  def handle_continue(:create_socket, state) do
    case create_socket(state.host, state.port) do
      {:ok, socket, port_number} ->
        Logger.debug("listening on port #{port_number}")
        # register myself in the registr
        Registry.register(
          state.node_id,
          state.node_id,
          :tcp_listener,
  ...
  @spec create_socket(hostname(), port_number()) ::
          {:ok, :inet.socket(), port_number()}
          | {:error, :failed_to_create_socket, any()}
  defp create_socket(host, port) do
    case :gen_tcp.listen(port, ifaddr: host, mode: :binary) do
      {:ok, listen_socket} ->
  ...
  # """
  @spec async_accept(:inet.socket(), Id.t()) :: :ok
  ...
  # """
  @spec wait_for_accept(:inet.socket(), Id.t()) ::
  ...
  # """
  @spec create_new_connection(:inet.socket(), t()) ::
          {:ok, pid()} | {:error, any()}
  defp create_new_connection(socket, node_id) do
    args = [node_id: node_id, socket: socket]

    # name of the supervisor under which the connection will be started
    supervisor = Registry.key(node_id, node_id, :tcp_supervisor)

    case DynamicSupervisor.start_child(supervisor, {TCP.Server, args}) do
      {:ok, pid} ->
        {:ok, pid}
